...
function ApproveToken({ tokenName, signer, tokenContract }: ApproveTokenProps) {
  ...
  const allowanceToken = async () => {
    ...
    try {
      ...
      const res = await tokenContract.allowance(signer.address, import.meta.env.VITE_LIQUIDITY_POOL_ADDRESS);
  ...
  const approveToken = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const tx = await tokenContract.approve(import.meta.env.VITE_LIQUIDITY_POOL_ADDRESS, ethers.parseUnits(approveAmount, 18));
